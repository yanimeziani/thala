"""Thala Backend - FastAPI application entry point."""
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    logger.info(f"Thala Backend started successfully on {settings.api_v1_prefix}")

    yield

    # Shutdown
    logger.info("Shutting down Thala Backend...")
    await engine.dispose()
    logger.info("Thala Backend shutdown complete")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        ...
        description="Thala - Amazigh cultural platform backend API",
        version="1.0.0",
        lifespan=lifespan,
        docs_url="/docs",
        redoc_url="/redoc",
